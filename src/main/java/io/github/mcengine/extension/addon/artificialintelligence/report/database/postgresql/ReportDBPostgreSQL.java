...
public class ReportDBPostgreSQL implements ReportDB {
     ...
     */
    public ReportDBPostgreSQL(Connection conn, MCEngineExtensionLogger logger) {
        this.conn = conn;
        this.logger = logger;
    }
                ...
                while (rs.next()) {
                    ...
                    if (text != null && !text.isBlank()) {
                        ...
                        try {
                            ...
                            if (logger != null) logger.warning("Failed to archive report ID=" + id + " (PostgreSQL): " + e.getMessage());
                        }
                    }
                }
            }
        } catch (SQLException e) {
            if (logger != null) logger.warning("Failed to fetch reasons (PostgreSQL): " + e.getMessage());
        }
    ...
    private void archiveReport(int reportId, String platform, String model, String reportText) throws SQLException {
        ...
        try {
            ...
            try (PreparedStatement backupStmt = conn.prepareStatement(backupSql);
                ...
                conn.commit();
                if (logger != null) logger.info("Archived and deleted report with ID: " + reportId + " (PostgreSQL).");
            } catch (SQLException e) {
                conn.rollback();
                throw e;
            } finally {
